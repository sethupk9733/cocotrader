...
function updateLiveTickerBar() {
  const pillEl = getEl('liveRatePill');
  if (!pillEl) return;
  pillEl.innerHTML = `
...
function openManageLotLabourModal(lotId) {
  ...
  if (bodyEl) {
    ...
    getEl('manageLotLabourForm')?.addEventListener('submit', (e) => {
      e.preventDefault();
      const checkedWorkerCbs = Array.from(document.querySelectorAll('input[name="manageLotWorkerAtt"]:checked'));

      const attendanceArray = checkedWorkerCbs.map(cb => {
        const wId = cb.value;
...
window.openGiveWorkerQuickCashModal = openGiveWorkerQuickCashModal;
window.openPayrollReviewModal = openPayrollReviewModal;
window.openWorkerPayrollModal = openPayrollReviewModal;
